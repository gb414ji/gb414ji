...
# 2) How to convert to PDF (quick commands)

Option 1 — using `pandoc` (recommended):

1. Save above Markdown to `ctf_notes.md`.
...
* Open `ctf_notes.md` in VS Code or similar, Print -> Save as PDF.

Option 3 — use `enscript` + `ps2pdf`:

```bash
apt install -y enscript ghostscript
enscript -p notes.ps ctf_notes.md
ps2pdf notes.ps ctf_notes.pdf
```

---

# 3) Plain-text quick version (if you prefer .txt)

If you want a plain text file instead, copy the same markdown but save as `ctf_notes.txt`. You can open and print to PDF from any editor.

---

If you want I can:

* Paste a ready-to-save `ctf_notes.md` file contents again,
* Or produce a smaller one-page summary,
* Or give the exact `pandoc` command you can run inside your container (if `pandoc` is installed).
